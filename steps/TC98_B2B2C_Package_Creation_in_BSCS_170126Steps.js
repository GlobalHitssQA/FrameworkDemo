...
Given('the user is authenticated in BSCS7 system with package configuration permissions', async function () {
...
When('the user navigates to B2B2C package creation module', async function () {
...
When('the user enters package data with capacity {string} cost {string} validity {string} and classification {string}', async function (capacity, cost, validity, classification) {
...
Then('the system validates and accepts the entered data', async function () {
  const isDataValid = await packagePage.isDataValidationSuccessful();
...
Then('the system registers coverage and queuing restrictions correctly', async function () {
  const areRestrictionsApplied = await packagePage.areRestrictionsApplied();
  expect(areRestrictionsApplied).toBeTruthy();
});

When('the user saves the B2B2C 120GB package configuration', async function () {
...
When('the user verifies the package in the available packages list for SOLD plan', async function () {
  await packagePage.navigateToPackageList();
  await packagePage.filterBySOLDPlan();
});

Then('the system displays the B2B2C 120GB package with 24 months validity in the catalog with all configured data', async function () {
  const isPackageVisible = await packagePage.isPackageInCatalog('B2B2C', '120GB', '24');
