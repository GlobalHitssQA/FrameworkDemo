...
...              configurada para mensajería no es válida o no está accesible
...
...              Precondiciones:
...
*** Variables ***
# Datos de prueba - Configuración de cola de mensajería
${COLA_RESPUESTA_INVALIDA}           queue.response.invalid.amigopaguitos
${NUMERO_TELEFONICO_VALIDO}           5512345678
${TRANSACTION_ID_ESB12}               TXN-2026-ESB12-MENSAJERIA-001
...
Validar Error ESB12 Por Cola De Respuesta No Válida En Mensajería
    ...
    ...                cuando la cola de respuesta configurada para mensajería no es válida o no existe.
    ...
    ...                Flujo de validación:
    ...                1. Configurar el ESB con una cola de respuesta no válida para mediaciones que usan mensajería
    ...                2. Enviar petición desde Amigo Paguitos que requiere uso de recursos de mensajería
    ...                3. Verificar que el ESB detecta que la cola de respuesta no es válida durante el procesamiento
    ...                4. Validar que el ESB genera el código de error ESB12 con descripción del problema
    ...                5. Verificar que el ESB ejecuta reintento automático según configuración
    ...                6. Revisar el log de infraestructura para identificar detalles del problema con la cola
    [Tags]    PruebaGeneradaIA    CasosDeError    ESB    Mensajeria    ColaRespuesta    IntegracionServicios

    # STEP 1: Configurar el ESB con una cola de respuesta no válida o inexistente para mediaciones que usan mensajería
    ...
    Cuando se envía una petición desde Amigo Paguitos que requiere uso de mensajería en el ESB
    ...    ${NUMERO_TELEFONICO_VALIDO}

    # STEP 3: Verificar que el ESB detecta que la cola de respuesta configurada no existe o no es accesible
    ...
    ...    ${TRANSACTION_ID_ESB12}

    # STEP 6 (validación): El log registra el nombre de la cola inválida, el error de acceso y las acciones de reintento
