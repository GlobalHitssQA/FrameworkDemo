...
Validar Manejo De Código De Error ESB2 Servicio No Disponible
    ...
    # GIVEN: Se prepara una solicitud válida de consulta con número telefónico correcto
    Cuando se prepara una solicitud válida de consulta con número telefónico correcto
    ...    ${NUMERO_TELEFONICO_VALIDO}

    # WHEN: Se simula la indisponibilidad del servicio BES (detener el servicio o bloquear la comunicación)
    Y se simula la indisponibilidad del servicio BES

    # THEN: Capa de Integración recibe la solicitud válida e intenta establecer conexión con BES
    Entonces Capa de Integración detecta que BES no está accesible

    # AND: El sistema devuelve código ESB2 con mensaje 'Servicio NO disponible'
    Y retorna el código de error ESB2 con mensaje Servicio NO disponible

    # AND: El flag de reintento está habilitado (S)
